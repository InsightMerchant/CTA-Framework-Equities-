# entry_exit_logic/two_factor_reverse_short_only.py
"""
Short-only reversal: only takes the SELL side (fade the high).
When dir_factor >= dir_long_thresh AND conf condition met → go SHORT.
Never longs.
"""
import numpy as np
import pandas as pd
...
def two_factor_reverse_short_only(
        df: pd.DataFrame,
    ...
    # Only the "fade the high" condition (long_signal in reverse = go SHORT)
    ...
    short_entry = long_dir_cond & conf_cond  # Fade the high
